...
To get your embed script, you'll need to go to your TrackPlay dashboard, select the video you want and then go to `Embed` section.
...
The TrackPlay script automatically initializes the player and dispatches a `TrackPlayReady` event when it's ready to use:
...
- **Always wait for `TrackPlayReady`** - Don't try to access the player before this event fires
